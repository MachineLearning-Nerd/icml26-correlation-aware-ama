...
def claim4_seed_stability() -> None:
    ...
    revenues = [float(row["caama_revenue"]) for row in rows]
    regrets = [float(row["caama_ir_regret"]) for row in rows]
    scales = [float(row["payment_scale"]) for row in rows]
    ...
    for seed, regret, scale in zip(seeds, regrets, scales):
        right.annotate(
            f"s={scale:.3f}",
            (seed, regret),
            xytext=(0, 7),
    ...
    right.set_ylim(0.00255, 0.00322)
    right.grid()
    right.legend(fontsize=8)
    fig.suptitle("Five independent pCor training seeds", fontweight="bold")
